...
class MatchesByLocation extends StatefulWidget {
  const MatchesByLocation({super.key});

  @override
  State<MatchesByLocation> createState() => _MatchesByLocationState();
}

class _MatchesByLocationState extends State<MatchesByLocation> {
  ...
  void initState() {
    ...
    _dashboardController.fetchMatchesbyLocationList();
  ...
  Widget build(BuildContext context) {
    return Obx(
      () {
        if (_dashboardController.matchesbyLocationList.isEmpty &&
            _dashboardController.matchesbylocationListfetching.value == false) {
          return RefreshIndicator(
            onRefresh: () {
              return Future(() async {
                _dashboardController.recommendedbyLocationpage.value = 1;
                _dashboardController.matchesByLocationhasMore.value = true;
                _dashboardController.matchesbyLocationList.clear();

                await _dashboardController.fetchMatchesbyLocationList();
              });
            },
            child: ListView(
              children: const [
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              shrinkWrap: true,
              itemCount: _dashboardController.matchesbyLocationList.length + 1,
              itemBuilder: (context, index) {
                // Check if the index is less than the data length
                if (index < _dashboardController.matchesbyLocationList.length) {
                  // Access the current match data
                  var matchData =
                      _dashboardController.matchesbyLocationList[index];
                              ...
                              child: SizedBox(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      " ${matchData["last_online_time"]}", // Example date, replace with dynamic value
                  ...
                  if (_dashboardController.matchesbyLocationList.isEmpty &&
                      _dashboardController
                          .matchesbylocationListfetching.value) {
                    return Padding(
                    ...
                    return GetBuilder<DashboardController>(
                      id: 'no_more_data_for_Location',
                      builder: (controller) {
                        return Padding(
                          padding: const EdgeInsets.all(18.0),
                          child: Center(
                              child: _dashboardController
                                      .matchesByLocationhasMore.value
                                  ? Shimmer.fromColors(
                                      baseColor: Colors.grey[300]!,
                                      highlightColor: Colors.grey[100]!,
                                      child: Container(
                                      ...
                                      child: Text.rich(
                                        ...
                                        TextSpan(
                                          children: [
                                            TextSpan(
                                                text:
                                                    "  No more profiles new profiles coming soon..! ",
                                                style: CustomTextStyle.title),
                                          ],
                                        ),
                                      ),
