...
class SearchByIDResultScreen extends StatefulWidget {
  const SearchByIDResultScreen({super.key});

  @override
  State<SearchByIDResultScreen> createState() => _SearchByIDResultScreenState();
}

class _SearchByIDResultScreenState extends State<SearchByIDResultScreen> {
  ...
  void initState() {
    ...
    controller.addListener(() {
      if (controller.position.maxScrollExtent == controller.offset) {
        ...
        _advancefiltercontroller.basicSearch();
      }
    });
  }
      ...
      body: SafeArea(
        ...
        children: [
          ...
          Obx(
            () {
              String? language = sharedPreferences?.getString("Language");
              if (_advancefiltercontroller.searchbyIDList.isEmpty) {
                return Center(
                  child: Text(
                    language == 'en'
                ...
                return ListView.builder(
                  ...
                  itemCount: _advancefiltercontroller.searchbyIDList.length + 1,
                  itemBuilder: (context, index) {
                    // Check if the index is less than the data length
                    if (index <
                        _advancefiltercontroller.searchbyIDList.length) {
                      // Access the current match data
                      var matchData =
                          _advancefiltercontroller.searchbyIDList[index];
